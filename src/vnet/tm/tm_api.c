...
vl_api_tm_sys_node_add_t_handler (vl_api_tm_sys_node_add_t *mp)
{
  ...
  rv = vnet_tm_sys_node_add (sw->hw_if_index, node_id, parent_node_id,
			     priority, weight, lvl, &n_p, flow_name);
  ...
  vl_api_tm_sys_node_shaper_update_t *mp)
{
  ...
  rv = vnet_tm_sys_node_shaper_update (sw->hw_if_index, node_id,
				       shaper_profile_id);
