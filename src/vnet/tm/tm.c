...
vnet_tm_create_flow_id (const char *flow_name)
{
  ...
  hash_set_mem (vnet_tm_flow_name_to_id_hash, format (0, "%s", flow_name),
		flow_id);
  return flow_id;
}
  ...
  if (vnet_tm_flow_name_to_id_hash == 0)
    {
      ...
      vnet_flow_get_range (vnm, "tm", VNET_TM_MAX_FLOWS,
			   &vnet_tm_flow_id_start);
    }
...
int
vnet_tm_sys_node_add (u32 hw_if_idx, u32 node_id, i32 parent_node_id,
		      ...
		      vnet_tm_node_params_t *params, const char *flow_name)
{
  ...
  rv = dev_class->vnet_tm_sys_impl->node_add (hw_if_idx, node_id,
					      parent_node_id, priority, weight,
...
int
vnet_tm_sys_shaper_profile_create (u32 hw_if_idx,
...
vnet_tm_sys_node_shaper_update (u32 hw_if_idx, u32 node_id, i32 shaper_id)
{
  ...
  rv = dev_class->vnet_tm_sys_impl->node_shaper_update (hw_if_idx, node_id,
							shaper_id);
...
vnet_tm_sys_shaper_profile_delete (u32 hw_if_idx, i32 shaper_id)
{
  ...
  rv =
    dev_class->vnet_tm_sys_impl->shaper_profile_delete (hw_if_idx, shaper_id);
...
vnet_tm_sys_node_sched_weight_update (u32 hw_if_idx, u32 node_id, u32 weight)
{
  ...
  rv = dev_class->vnet_tm_sys_impl->node_sched_weight_update (hw_if_idx,
							      node_id, weight);
...
int
vnet_tm_sys_node_read_stats (u32 hw_if_idx, u32 node_idx,
			     vnet_tm_stats_params_t *param)
{
  ...
  rv =
    dev_class->vnet_tm_sys_impl->node_read_stats (hw_if_idx, node_idx, param);
...
int
vnet_tm_sys_level_get_capabilities (u32 hw_if_idx,
				    vnet_tm_level_capa_params_t *param,
				    u32 lvl)
{
  vnet_main_t *vnm = vnet_get_main ();
  vnet_hw_interface_t *hi = vnet_get_hw_interface (vnm, hw_if_idx);
  ...
  rv = dev_class->vnet_tm_sys_impl->tm_level_get_capabilities (hw_if_idx,
							       param, lvl);
