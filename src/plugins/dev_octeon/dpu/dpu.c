...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left >= 8)
    {
      vlib_buffer_advance (b[0], OCT_H2D_META_SIZE);
      vlib_buffer_advance (b[1], OCT_H2D_META_SIZE);
      vlib_buffer_advance (b[2], OCT_H2D_META_SIZE);
      vlib_buffer_advance (b[3], OCT_H2D_META_SIZE);
	...
	vlib_add_trace (vm, node, b[0], sizeof (u32));
      if (b[1]->flags & VLIB_BUFFER_IS_TRACED)
	vlib_add_trace (vm, node, b[1], sizeof (u32));
      if (b[2]->flags & VLIB_BUFFER_IS_TRACED)
	vlib_add_trace (vm, node, b[2], sizeof (u32));
      if (b[3]->flags & VLIB_BUFFER_IS_TRACED)
	vlib_add_trace (vm, node, b[3], sizeof (u32));
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  oct_d2h_meta_t *hdr0, *hdr1, *hdr2, *hdr3;
  u32 n_left, next0, next1, next2, next3;
  vlib_buffer_t *bufs[VLIB_FRAME_SIZE];
  u16 nexts[VLIB_FRAME_SIZE], *next;
  ...
  while (n_left >= 8)
    {
      ...
      vlib_buffer_advance (b[0], -OCT_D2H_META_SIZE);
      vlib_buffer_advance (b[1], -OCT_D2H_META_SIZE);
      vlib_buffer_advance (b[2], -OCT_D2H_META_SIZE);
      vlib_buffer_advance (b[3], -OCT_D2H_META_SIZE);
