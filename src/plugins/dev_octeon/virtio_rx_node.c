			   ...
			   vnet_dev_rx_queue_t *rxq)
{
  ...
  while (rx_q_map)
	...
	trace_count -= oct_virt_trace_rx_buffers (
	  vm, node, b, n_rx_pkts, trace_count, q_map, virtio_id, next_index);
      ...
      if (PREDICT_TRUE (next_index == VNET_DEV_ETH_RX_PORT_NEXT_ETH_INPUT))
	{
	  ...
	  nf = vlib_node_runtime_get_next_frame (
	    vm, node, vnet_dev_get_rx_queue_if_sw_if_index (rxq));
	  ...
	  ef->sw_if_index = vnet_dev_get_rx_queue_if_sw_if_index (rxq);
  ...
  vlib_increment_combined_counter (
    ...
    thr_idx, vnet_dev_get_rx_queue_if_hw_if_index (rxq), rx_pkts_total,
    n_rx_bytes);
