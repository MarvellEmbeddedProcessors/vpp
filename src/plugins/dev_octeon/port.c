...
oct_port_init (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  foreach_vnet_dev_port_args (arg, port)
	...
	is_allmulti_enable = true;
      else if (arg->id == OCT_PORT_ARG_EN_ETH_PAUSE_FRAME &&
	       vnet_dev_arg_get_bool (arg))
	is_flow_ctrl_enable = true;
      else if (arg->id == OCT_PORT_ARG_SWITCH_HDR_TYPE &&
  ...
  if (inl_main->inl_dev)
    {
      ...
      if (rrv)
	{
	  rv = oct_roc_err (dev, rrv, "roc_nix_fc_config_set failed");
	  oct_port_deinit (vm, port);
	  return rv;
	}
    }
...
oct_port_stop (vlib_main_t *vm, vnet_dev_port_t *port)
{
  ...
  if (inl_main->inl_dev)
    {
      if ((rrv = roc_nix_inl_rq_ena_dis (nix, false)))
	{
	  oct_roc_err (dev, rrv, "roc_nix_inl_rq_ena_dis failed");
	  return;
	}
    }
...
vnet_dev_rv_t
oct_op_config_link_speed (vlib_main_t *vm, vnet_dev_port_t *port,
			  u32 link_speed_kbps)
{
  ...
  log_debug (dev,
	     ...
	     "advertising 0x%lx",
	     link_info.speed, link_info.autoneg, link_info.full_duplex,
  ...
  if (rrv)
    {
      vnet_dev_rv_t rv =
	oct_roc_err (dev, rrv, "roc_nix_mac_link_info_set() failed");
