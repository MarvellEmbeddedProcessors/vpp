 ...
 */
#ifndef __included_http_static_l4_h__
#define __included_http_static_l4_h__
...
#define HTTP_STAT_LINE_LEN 64
#define HTTP_PATH_LEN	   10
...
} http_static_l4_main_t;

extern http_static_l4_main_t http_static_l4_main;
...
typedef enum
{
  ...
  /** Session has more data to receive */
  HTTP_STATE_RECEIVE_MORE_DATA,
  /** Session has sent an OK response */
  HTTP_STATE_OK_SENT,
  /** Session has sent an HTML response */
  HTTP_STATE_SEND_MORE_DATA,
  /** Session has sent file, closes the connection */
  HTTP_STATE_CLOSING,
  /** Number of states */
  HTTP_STATE_N_STATES,
...
typedef struct
{
  ...
  /** Per thread Session pool reader writer lock */
  clib_rwlock_t *thr_sessions_lock;
  ...
  /** filename for index.html redirect */
  char redirect_file_name[HTTP_PATH_LEN];
  ...
  u32 fifo_size;
  /** fifo dequeue threshold */
  u32 fifo_deq_thresh;
  /** The bind URI, defaults to tcp://0.0.0.0/80 */
  u8 *uri;
  /** Keep Connection Alive after file fetch is done, default=0 */
  u8 keepalive;
  vlib_main_t *vlib_main;
} http_static_l4_server_main_t;

extern http_static_l4_server_main_t http_static_l4_server_main;

int http_static_l4_server_enable (u32 fifo_size, u32 cache_limit,
				  u32 prealloc_fifos, u32 private_segment_size,
				  u8 *www_root, u8 *uri);

void http_static_l4_server_register_builtin_handler (void *fp, char *url,
						     int type);

#endif /* __included_http_static_l4_h__ */
