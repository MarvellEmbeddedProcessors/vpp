...
function setup_devices() {
	...
	# SSO and NPA devices
	sso_pf=${SSO_DEV:-$(lspci -d :a0f9 | tail -1 | awk '{ print $1 }')}
	npa_pf=${NPA_DEV:-$(lspci -d :a0fb | tail -1 | awk '{ print $1 }')}
	devs+=" $sso_pf"
	devs+=" $npa_pf"

	# DMA device
	dma_pf=$(lspci -d :a080 | tail -1 | awk '{ print $1 }')
	if [[ -e /sys/bus/pci/devices/$dma_pf/sriov_numvfs ]]; then
		...
		dma_vf=$(lspci -d :a081 | tail -1 | awk '{ print $1 }')
		devs+=" $dma_vf"
	fi

	if [[ $CPU == "cn10ka" ]]; then
		inl_pf=${INL_DEV:-$(lspci -d :a0f0 | tail -1 | awk '{ print $1 }')}
		devs+=" $inl_pf"
	fi

	if [[ $CPU == "cn10kb" ]]; then
		nix_pfs=${ETH_DEV:-$(lspci -d :a063 | tail -1 | awk '{ print $1 }')}
		devs+=" $nix_pfs"
		inl_pf=${INL_DEV:-$(lspci -d :a0f0 | tail -1 | awk '{ print $1 }')}
		devs+=" $inl_pf"
	fi
	...
	for d in $(lspci -d :a0f9 | awk '{ print $1 }'); do
...
if [[ -n ${TARGET_BOARD:-} ]]; then
	...
	$TARGET_SCP_CMD $SCRIPTPATH/$SCRIPTNAME $TARGET_BOARD:$REMOTE_DIR/oct-target-setup.sh
	VFIO_DEVBIND=${VFIO_DEVBIND:-$REMOTE_BUILD_DIR/ci/test/board/oxk-devbind-basic.sh}
	TARGET_EXPORTS="VFIO_DEVBIND=$VFIO_DEVBIND HP=$HP"
	$TARGET_SSH_CMD $TARGET_BOARD \
		"$SUDO $TARGET_EXPORTS $REMOTE_DIR/oct-target-setup.sh"
	exit 0
fi
