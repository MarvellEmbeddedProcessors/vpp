...
class CIRunner:
    def __init__(self, dir, dpdk_devbind, verb, dry_run):
        ...
        self.cptpfdev = None
        self.test_dir = dir
        self.dpdk_devbind = dpdk_devbind
        self.drun = dry_run
        self.verb = verb
    ...
    def bind_driver(self, driver, dev_bdf):
        if self.arch_model == "_10xx":
            cmdstr = f"{self.dpdk_devbind} -b {driver} {dev_bdf}"
            self.run_cmd(cmdstr)
        ...
        for line in lspci.stdout:
            if dbdparse in line:
                ...
                if lnparse[0] != self.evtpfbdf.encode("utf-8"):
                    ...
                    self.run_cmd(cmdstr)
                    # cmdstr = "echo %s > /sys/bus/pci/driver_probe" % devbdf
                    # self.run_cmd(cmdstr)
...
if __name__ == "__main__":
    ...
    dpdk_devbind = os.environ.get("DPDK_DEVBIND")
